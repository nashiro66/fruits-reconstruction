...
class PrbNeeVolumeIntegrator(mi.ad.integrators.common.RBIntegrator):
  ...
  def sample(
    ...
    while dr.hint(
      ...
      with dr.resume_grad(when=not primal):
        ...
        Lr_dir = throughput_mis * bsdf_val * em_weight
...
def register():
  ...
  mi.register_integrator(
      "prb_nee_volume", lambda props: PrbNeeVolumeIntegrator(props)  # pylint: disable=unnecessary-lambda
  )
